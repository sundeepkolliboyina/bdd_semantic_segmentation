...
def gen_batch_function(data_folder, image_shape):
    ...
    def get_batches_fn(batch_size):
        ...
        label_paths = {
            re.sub(r'_(drivable_color).png', '.jpg', os.path.basename(path)): path
            for path in glob(os.path.join(data_folder, 'gt_image', '*drivable_color.png'))}
        #SK - Red background_color = np.array([255, 0, 0])
        #SK - Black background is 0, 0, 0
        ...
        for batch_i in range(0, len(image_paths), batch_size):
            ...
            for image_file in image_paths[batch_i:batch_i+batch_size]:
                ...
                if gt_image.shape[2] == 3:
                    ...
                    gt_bg = np.all(gt_image == background_color, axis=0)
                    gt_bg = gt_bg.reshape(*gt_bg.shape, 1)
                    gt_image = np.concatenate((gt_bg, np.invert(gt_bg)), axis=0)
...
def save_inference_samples(runs_dir, data_dir, sess, image_shape, logits, keep_prob, input_image, epoch_nr):
    ...
    #SK subfolder = "epoch" + str(epoch_nr) + "_" + str(time.time())
